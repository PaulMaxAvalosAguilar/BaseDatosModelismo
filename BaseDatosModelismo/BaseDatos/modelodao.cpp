...
void ModeloDao::init() const
{
    if (!mDatabase.tables().contains("Modelo")) {
        ...
        query.exec("CREATE TABLE Modelo (id INTEGER PRIMARY KEY AUTOINCREMENT, marca TEXT,codigo TEXT,nombre TEXT,escala TEXT,numeroUnidades INTEGER)");
...
{
    QSqlQuery query(mDatabase);
    query.prepare("UPDATE Modelo (marca,codigo,nombre,escala,numeroUnidades) = (:marca,:codigo,:nombre,:escala,:numeroUnidades) WHERE  id = (:id)");
    query.bindValue(":marca", record.getMarca());
    query.bindValue(":codigo", record.getCodigo());
    query.bindValue(":nombre", record.getNombre());
    query.bindValue(":escala", record.getEscala());
...
std::unique_ptr<Modelo> ModeloDao::getRecord(int recordId) const
{
    QSqlQuery query(mDatabase);
